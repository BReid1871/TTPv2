...
/** Renders the /logs battle-history page: one row per saved battle, with
 * links to the raw files and a per-battle zip download -- see
 * src/logging/battleLogger.ts for what each file contains. */
    ...
    .map((s) => {
      ...
      return `<tr>
        ...
        <td class="links">
          ...
          <a href="/logs/${s.id}/recommendations.json${qs}">recommendations.json</a>
        </td>
      </tr>`;
    })
    .join('\n');
